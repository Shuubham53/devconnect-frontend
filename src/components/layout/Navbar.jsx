      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px', display: 'flex', alignItems: 'center', height: '54px', gap: '8px' }}>
        ...
        <div className="desktop-nav" style={{ display: 'flex', alignItems: 'center', gap: '2px', flex: 1 }}>
          {navLinks.map(({ label, path }) => (
            <Link key={path} to={path} style={{ textDecoration: 'none' }}>
              <div style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '13px', fontWeight: isActive(path) ? '600' : '400', color: isActive(path) ? '#eff1f6' : '#94a3b8', background: isActive(path) ? '#3d3d3d' : 'transparent', transition: 'all 0.15s', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0, marginLeft: 'auto' }}>
          ...
          <Link to="/search" style={{ textDecoration: 'none' }} className="desktop-only">
            <div style={{ padding: '7px 8px', borderRadius: '6px', cursor: 'pointer', color: '#94a3b8', transition: 'all 0.15s' }}
          ...
          <Link to="/notifications" style={{ textDecoration: 'none' }} className="desktop-only">
            <div style={{ position: 'relative', padding: '7px 8px', borderRadius: '6px', cursor: 'pointer', color: '#94a3b8', transition: 'all 0.15s' }}
              onMouseEnter={e => { e.currentTarget.style.background = '#3d3d3d'; e.currentTarget.style.color = '#eff1f6' }}
              onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#94a3b8' }}
            >
              <Bell size={17} />
          ...
          <div style={{ position: 'relative', marginLeft: '4px' }} onClick={e => { e.stopPropagation(); setDropdownOpen(!dropdownOpen) }}>
            ...
            >
              <div style={{ width: '28px', height: '28px', borderRadius: '4px', background: 'rgba(255,161,22,0.15)', border: '1px solid rgba(255,161,22,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', color: '#ffa116', overflow: 'hidden', flexShrink: 0 }}>
              ...
              <ChevronDown size={12} color='#94a3b8' className="desktop-only" />
            </div>

            {dropdownOpen && (
              <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, background: '#282828', border: '1px solid #3d3d3d', borderRadius: '8px', padding: '4px', minWidth: '160px', zIndex: 200, boxShadow: '0 8px 24px rgba(0,0,0,0.5)' }}>
                ...
                {[
                  ...
                  { label: 'Create Post', action: () => { navigate('/create-post'); setDropdownOpen(false) } },
                ].map(item => (
                  <div key={item.label} onClick={item.action} style={{ padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: '#94a3b8', transition: 'all 0.15s' }}
                    onMouseEnter={e => { e.currentTarget.style.background = '#3d3d3d'; e.currentTarget.style.color = '#eff1f6' }}
                    onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#94a3b8' }}
                  >
                    {item.label}
                ...
                <div onClick={handleLogout} style={{ padding: '8px 12px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: '#ef4444', transition: 'all 0.15s' }}
                  onMouseEnter={e => e.currentTarget.style.background = '#3d3d3d'}
